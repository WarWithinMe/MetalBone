...
namespace MetalBone
{
	has_slots::has_slots(const has_slots& hs)
	{
		...
		while(it != itEnd)
		{
			(*it)->duplicate_slot(&hs, this);
			m_senders.insert(*it);
			++it;
		}
	}

	has_slots::~has_slots() { disconnect_all(); }
	...
	void has_slots::signal_connect(SignalBase* sender)
	{
		lock_block lock(&m_mutex);
		m_senders.insert(sender);
	}

	void has_slots::signal_disconnect(SignalBase* sender)
	{
		lock_block lock(&m_mutex);
